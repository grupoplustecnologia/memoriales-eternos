...
const pageSlug = 'cementerio-virtual-gatos-espana';
const canonical = getCanonicalUrl(`/${pageSlug}`);
const ogImage = getOgImageUrl(pageSlug, 'landing');

const breadcrumbs = [{ name: 'Home', url: 'https://memorias-eternas.app' }, { name: 'Cementerio Virtual Gatos Espana', url: canonical }];
...
export default function CementerioVirtualGatosEspana() {
  const localBusinessSchema = generateLocalBusinessSchema('Cementerio Virtual Gatos Espana');
  const breadcrumbSchema = generateBreadcrumbSchema(breadcrumbs);
  const faqSchema = generateFAQSchema(faqs);
  const webPageSchema = generateWebPageSchema('Cementerio Virtual Gatos Espana - Forever Pet Friend', 'Memorial para mascotas.', canonical, ogImage);
