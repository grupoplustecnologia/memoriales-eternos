...
const pageSlug = 'cementerio-virtual-animales-domesticos';
const canonical = getCanonicalUrl(`/${pageSlug}`);
const ogImage = getOgImageUrl(pageSlug, 'landing');

const breadcrumbs = [{ name: 'Home', url: 'https://memorias-eternas.app' }, { name: 'Cementerio Virtual Animales Domesticos', url: canonical }];
...
export default function CementerioVirtualAnimalesDomesticos() {
  const localBusinessSchema = generateLocalBusinessSchema('Cementerio Virtual Animales Domesticos');
  const breadcrumbSchema = generateBreadcrumbSchema(breadcrumbs);
  const faqSchema = generateFAQSchema(faqs);
  const webPageSchema = generateWebPageSchema('Cementerio Virtual Animales Domesticos - Forever Pet Friend', 'Memorial para mascotas.', canonical, ogImage);
