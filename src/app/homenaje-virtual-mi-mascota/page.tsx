...
const pageSlug = 'homenaje-virtual-mi-mascota';
const canonical = getCanonicalUrl(`/${pageSlug}`);
const ogImage = getOgImageUrl(pageSlug, 'landing');

const breadcrumbs = [{ name: 'Home', url: 'https://memorias-eternas.app' }, { name: 'Homenaje Virtual Mi Mascota', url: canonical }];
...
export default function Page() {
  const localBusinessSchema = generateLocalBusinessSchema('Homenaje Virtual Mi Mascota');
  const breadcrumbSchema = generateBreadcrumbSchema(breadcrumbs);
  const faqSchema = generateFAQSchema(faqs);
  const webPageSchema = generateWebPageSchema('Homenaje Virtual Mi Mascota - Forever Pet Friend', 'Memorial para mascotas.', canonical, ogImage);
