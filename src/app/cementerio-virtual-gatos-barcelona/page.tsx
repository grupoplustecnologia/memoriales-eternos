...
const pageSlug = 'cementerio-virtual-gatos-barcelona';
const canonical = getCanonicalUrl(`/${pageSlug}`);
const ogImage = getOgImageUrl(pageSlug, 'landing');

const breadcrumbs = [{ name: 'Home', url: 'https://memorias-eternas.app' }, { name: 'Cementerio Virtual Gatos Barcelona', url: canonical }];
...
export default function CementerioVirtualGatosBarcelona() {
  const localBusinessSchema = generateLocalBusinessSchema('Cementerio Virtual Gatos Barcelona');
  const breadcrumbSchema = generateBreadcrumbSchema(breadcrumbs);
  const faqSchema = generateFAQSchema(faqs);
  const webPageSchema = generateWebPageSchema('Cementerio Virtual Gatos Barcelona - Forever Pet Friend', 'Memorial para mascotas.', canonical, ogImage);
