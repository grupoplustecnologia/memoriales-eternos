...
const pageSlug = 'cementerio-virtual-hamsters';
const canonical = getCanonicalUrl(`/${pageSlug}`);
const ogImage = getOgImageUrl(pageSlug, 'landing');

const breadcrumbs = [{ name: 'Home', url: 'https://memorias-eternas.app' }, { name: 'Hamsters', url: canonical }];
const faqs = [{ question: '¿Puedo crear un memorial para mi hamster?', answer: 'Sí, honra la vida de tu hamster.' }, { question: '¿Es gratis?', answer: 'Sí, completamente.' }, { question: '¿Puedo compartir?', answer: 'Sí.' }];

export default function CementerioVirtualHamsters() {
  const localBusinessSchema = generateLocalBusinessSchema('Cementerio Virtual para Hamsters');
  const breadcrumbSchema = generateBreadcrumbSchema(breadcrumbs);
  const faqSchema = generateFAQSchema(faqs);
  const webPageSchema = generateWebPageSchema('Cementerio Virtual para Hamsters - Forever Pet Friend', 'Memorial para hamsters.', canonical, ogImage);
  return (
