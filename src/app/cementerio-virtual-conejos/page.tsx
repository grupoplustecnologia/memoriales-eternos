...
const pageSlug = 'cementerio-virtual-conejos';
const canonical = getCanonicalUrl(`/${pageSlug}`);
const ogImage = getOgImageUrl(pageSlug, 'landing');

const breadcrumbs = [{ name: 'Home', url: 'https://memorias-eternas.app' }, { name: 'Conejos', url: canonical }];
const faqs = [{ question: '¿Puedo crear un memorial para mi conejo?', answer: 'Sí, honra a tu conejo con un memorial permanente.' }, { question: '¿Es gratis?', answer: 'Sí.' }, { question: '¿Puedo compartir?', answer: 'Sí.' }];

export default function CementerioVirtualConejos() {
  const localBusinessSchema = generateLocalBusinessSchema('Cementerio Virtual para Conejos');
  const breadcrumbSchema = generateBreadcrumbSchema(breadcrumbs);
  const faqSchema = generateFAQSchema(faqs);
  const webPageSchema = generateWebPageSchema('Cementerio Virtual para Conejos - Forever Pet Friend', 'Memorial para conejos.', canonical, ogImage);
  return (
