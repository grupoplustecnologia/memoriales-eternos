...
const pageSlug = 'cementerio-virtual-hurones';
const canonical = getCanonicalUrl(`/${pageSlug}`);
const ogImage = getOgImageUrl(pageSlug, 'landing');

const breadcrumbs = [{ name: 'Home', url: 'https://memorias-eternas.app' }, { name: 'Hurones', url: canonical }];
const faqs = [{ question: '¿Memorial para hurones?', answer: 'Sí, para todos tus mascotas.' }, { question: '¿Gratis?', answer: 'Sí.' }, { question: '¿Compartir?', answer: 'Sí.' }];

export default function CementerioVirtualHurones() {
  const localBusinessSchema = generateLocalBusinessSchema('Cementerio Virtual para Hurones');
  const breadcrumbSchema = generateBreadcrumbSchema(breadcrumbs);
  const faqSchema = generateFAQSchema(faqs);
  const webPageSchema = generateWebPageSchema('Cementerio Virtual para Hurones - Forever Pet Friend', 'Memorial para hurones.', canonical, ogImage);
  return (
