...
export default function ProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [location, setLocation] = useState<string>('España');
  ...
  // Fetch profile from API - supports both ID and slug
  useEffect(() => {
    const fetchProfile = async () => {
      try {
        ...
        if (result.success && result.data) {
          ...
          let found = result.data.find((p: AnimalProfile) => p.id === id);
          if (!found) {
            found = result.data.find((p: AnimalProfile) => p.slug === id);
          }
          setProfile(found || null);

          // Get location from coordinates if available
          if (found && found.latitude && found.longitude) {
            reverseGeocode(found.latitude, found.longitude);
          }
      ...
      <div className="container mx-auto px-4 py-6 sm:py-8 md:py-12">
        ...
        <div className="max-w-4xl mx-auto space-y-4 sm:space-y-6">
          ...
          <Card>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="flex flex-wrap gap-3">
                <LikesButton profileId={id} />
                <ReactionsPanel profileId={id} />
                <ShareButton profileId={id} title={profile.name} />
              </div>
            </CardContent>
          </Card>
          ...
          <Card>
            ...
            <CardContent>
              <TagsManager profileId={id} isOwner={false} />
            </CardContent>
          </Card>
