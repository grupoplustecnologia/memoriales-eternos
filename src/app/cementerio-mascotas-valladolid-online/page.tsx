...
const pageSlug = 'cementerio-mascotas-valladolid-online';
const canonical = getCanonicalUrl(`/${pageSlug}`);
const ogImage = getOgImageUrl(pageSlug, 'landing');

const breadcrumbs = [{ name: 'Home', url: 'https://memorias-eternas.app' }, { name: 'Cementerio Mascotas Valladolid Online', url: canonical }];
...
export default function Page() {
  const localBusinessSchema = generateLocalBusinessSchema('Cementerio Mascotas Valladolid Online');
  const breadcrumbSchema = generateBreadcrumbSchema(breadcrumbs);
  const faqSchema = generateFAQSchema(faqs);
  const webPageSchema = generateWebPageSchema('Cementerio Mascotas Valladolid Online - Forever Pet Friend', 'Memorial para mascotas.', canonical, ogImage);
