...
const pageSlug = 'cementerio-virtual-mascotas-madrid';
const canonical = getCanonicalUrl(`/${pageSlug}`);
const ogImage = getOgImageUrl(pageSlug, 'landing');

const breadcrumbs = [{ name: 'Home', url: 'https://memorias-eternas.app' }, { name: 'Cementerio Virtual Mascotas Madrid', url: canonical }];
...
export default function CementerioVirtualMascotasMadrid() {
  const localBusinessSchema = generateLocalBusinessSchema('Cementerio Virtual Mascotas Madrid');
  const breadcrumbSchema = generateBreadcrumbSchema(breadcrumbs);
  const faqSchema = generateFAQSchema(faqs);
  const webPageSchema = generateWebPageSchema('Cementerio Virtual Mascotas Madrid - Forever Pet Friend', 'Memorial para mascotas.', canonical, ogImage);
